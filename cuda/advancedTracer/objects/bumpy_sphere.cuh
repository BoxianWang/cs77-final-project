...
class bumpy_sphere : public sphere {
  ...
  private:
    ...
    __device__ vec3 get_displaced_normal(point3 p, vec3 normal) const{
      ...
      float u, v;
      get_uv(p, u, v, radius);
      auto bu = 0.2*radius*(noise.turb(10*get_p(u+eps, v, radius)) - noise.turb(10*get_p(u, v, radius)))/eps;
      auto bv = 0.2*radius*(noise.turb(10*get_p(u, v+eps, radius)) - noise.turb(10*get_p(u, v, radius)))/eps;
